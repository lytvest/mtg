...
import ru.bdm.mtg.conditions.{Condition, Discard, InGraveyard, IsPlayFromHandAndMana}
...
case class DragonBreath(target: Option[Card] = None) extends Card {
  ...
  override val description: Map[Condition, Action] = Map(
    ...
    InGraveyard(this) -> Aura(target => DragonBreath(Some(target))) * RemoveFromGraveyard(this),
